...
export default function FeaturedProducts() {
  ...
  useEffect(() => {
      ...
      const fetchProducts = async () => {
        const response = await fetch("http://127.0.0.1:8000/api/get-vehicle");
    ...
    return (
      <section className="featured-product">
        <div className="container-fluid">
          <div className="featured-heading"><h2>All Vehicles  </h2></div>
          <div className="swiper-featured-product">
              ...
              {/* Repeat slides as needed */}
                {products.length > 0 ? (
                  ...
                  <img
                    src={`http://127.0.0.1:8000/${JSON.parse(product.album.replace(/\\/g, ''))}`}
                    alt={product.name}
                  />
