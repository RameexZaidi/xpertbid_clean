...
const onVehicleChange = e => {
  ...
  const handleInputChange = async (e) => {
    ...
    if (name === "category_id") {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/get-subcategories/${value}`
        );
    ...
    if (name === "sub_category_id") {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/get-childern/${value}`
        );
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    const auctionRes = await axios.post(
      "http://127.0.0.1:8000/api/auctions_store",
      auctionFd,
      {
        headers: {
  ...
  console.log("🚀 Firing property-verifications API…");
     ...
     await axios.post(
       "http://127.0.0.1:8000/api/property-verifications",
       propFd,
       { headers: {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${session.user.token}`,
    ...
    if (formData.category_id === "311") {
      ...
      const vehicleRes = await axios.post(
        "http://127.0.0.1:8000/api/vehicle-verifications",
        vehicleFd,
        {
          headers: {
            "Content-Type": "multipart/form-data",
