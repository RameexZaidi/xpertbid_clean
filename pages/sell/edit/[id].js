...
const EditAuction = () => {
  ...
  useEffect(() => {
       console.log("AUCTION API:", `http://127.0.0.1:8000/api/auctions/${id}`);

    if (!id || !session?.user?.token) return;
    axios.get(`http://127.0.0.1:8000/api/auctions/${id}`, {
...
const fullAlbum = albumArr.map(img =>
  img.startsWith("http")
    ? img
    : `http://127.0.0.1:8000/${img.startsWith("/") ? img.slice(1) : img}`
);
...
const fullDocs = docs.map(doc => 
  `http://127.0.0.1:8000/${doc.startsWith('/') ? doc.slice(1) : doc}`
);
...
setVehiclePreview(fullVDocs);
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:8000/api/get-countries")
      .then(res => setCountries(res.data.country));
  }, []);

  useEffect(() => {
    if (!formData.category_id) return;
    axios.get(`http://127.0.0.1:8000/api/get-subcategories/${formData.category_id}`)
      .then(res => setSubCategories(res.data.subcategories));
  }, [formData.category_id]);

  useEffect(() => {
    if (!formData.sub_category_id) return;
    axios.get(`http://127.0.0.1:8000/api/get-childern/${formData.sub_category_id}`)
  ...
  if (formData.category_id === "311" && vehicleDocuments.length === 0 && vehiclePreview.length === 0) {
    ...
    return;
       ...
       await axios.post(
      `http://127.0.0.1:8000/api/auctions_update/${id}`,
      auctionFd,
      {
        headers: {
...
{formData.category_id === "222" && (
  ...
  {albumFiles.length > 0 ? (
    ...
    oldAlbum.map((src, idx) => (
      <div key={idx} className="position-relative">
        <img
          src={src.startsWith('http') ? src : `http://127.0.0.1:8000/${src.startsWith('/') ? src.slice(1) : src}`}
